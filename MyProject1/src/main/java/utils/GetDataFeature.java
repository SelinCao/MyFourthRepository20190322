...
public class GetDataFeature {
     ...
     * 批量获取图片特征值
     *
     * @param jsonInfo jsonInfo中的JSONObject中有图片名和图片二进制数据
     * @param url      批量获取特征值接口url
    ...
    public static String getFeatureBatchMapLY(Map<String, byte[]> picMap, String faceUrl) {
        CloseableHttpClient client = HttpClients.createDefault();
        HttpPost httpPost = new HttpPost(faceUrl);
        MultipartEntityBuilder entity = MultipartEntityBuilder.create();
        for (Map.Entry<String, byte[]> entry : picMap.entrySet()) {
            entity.addBinaryBody("imageDatas", entry.getValue(), ContentType.DEFAULT_BINARY, entry.getKey());
...
//            }
    ...
    public static String getFeatureBatchMap(HashMap<String, Result> picMap, String faceUrl) {
        CloseableHttpClient client = HttpClients.createDefault();
        HttpPost httpPost = new HttpPost(faceUrl);
        MultipartEntityBuilder entity = MultipartEntityBuilder.create();
        for (Map.Entry<String, Result> entry : picMap.entrySet()) {
            ...
            if (pic != null && pic.length > 0) {
                ...
                System.out.println(entry.getValue().getValue(Bytes.toBytes("ATTR"), Bytes.toBytes("PERSON_NAME"))
                        + "获取图片失败！");
            }
        }
        httpPost.setEntity(entity.build());
    ...
    public static void main(String[] args) {
        String filepath = "E:\\testlsh\\1.jpg";
        String url = "http://10.45.144.94:80/verify/feature/gets";
        String result = GetDataFeature.getImageFeature(filepath, url);
