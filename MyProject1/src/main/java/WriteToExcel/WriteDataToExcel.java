...
public class WriteDataToExcel {
    private static final Logger LOGGER = LoggerFactory.getLogger(WriteDataToExcel.class.getName());
     ...
     * @param titles    标题串
     * @param values    内容集
     * @param pictitles 图片名称
     * @return True\False
     */
    private static boolean generateWorkbook(String path, String name, String style, List<String> titles, List<Map<String, Object>> values, List<String> pictitles) {
        ...
        while (iterator.hasNext()) {
            ...
            for (Map.Entry<String, Object> map : value.entrySet()) {
                ...
                if (pictitles.contains(title)) {
                    //声明一个画图的顶级管理器
                    HSSFPatriarch patriarch = sheet.createDrawingPatriarch();
                    insertImage(workbook, patriarch, (byte[]) object, cell.getRowIndex(), cell.getColumnIndex(), 1);
...
//        }
    ...
    private static Map<String, CellStyle> createStyles(Workbook wb) {
        ...
        cellStyleFont.setFontHeightInPoints((short) 16);
...
//        cellStyleB.setFont(cellStyleFont);
    ...
    public static void main(String[] args) {
        String path = "D:\\project\\writeDataTotal\\test.xlsx";
        String picPath = "D:\\project\\不超过64KB的照片\\数据产品开发部\\";
        String name = "sheet1";
...
//                picPaths.add(f.getName());
//            } else if (f.isDirectory()) {
//                picPaths = listALlFile(f, picPaths);
//            }
//        }

        for (int i = 0; i < fa.length; i++) {
            Random rand = new Random();
            //  String person_name = getRandomString(rand.nextInt(10) + 2);
            Map<String, Object> map = Maps.newHashMap();
            File fs = fa[i];
            map.put("人脸图片名", fs.getName());
            map.put("姓名", fs.getName().substring(0, fs.getName().lastIndexOf(".")));
            map.put("身份证号", "32062120100101");

            values.add(map);
        }
        //System.out.println(writerExcel(path, name, titles, values));
        System.out.println(writerExcel(path, name, titles, values, pictitles));
    }
}
